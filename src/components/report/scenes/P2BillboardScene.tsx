    ...
    <BaseScene onNext={onNext} sceneName={sceneName}>
      ...
      <div className="z-10">
        <div style={{ paddingLeft: '34px', paddingRight: '34px', paddingTop: '120px' }}>
          ...
          <div style={{ paddingTop: '60px', paddingBottom: '2px' }} hidden={!billboardCount}>
            <span className="text-r-pink" style={{ fontSize: '18px' }}>{billboardCount}</span> 个提问登上了热榜
          </div>
          <div className="text-lg mt-5" hidden={!billboardQuestionDate || !billboardQuestionTitle}>
            <span className="text-r-green" style={{ fontSize: '18px' }}>{billboardQuestionDate}</span>,
            <span className="text-r-fern" style={{ fontSize: '20px' }}>「{billboardQuestionTitle}」</span> 在热榜被更多人看见
          </div>
        </div>
        <div
        ...
        >
          {/* 1. 蓝色球 (没顶到时显示，顶到后隐藏) */}
          <div
            className={`absolute inset-0 transition-opacity duration-100 ${hasHit ? 'opacity-0' : 'opacity-100'}`}
          >
            <Image
          ...
          <div
            className={`absolute inset-0 ${hasHit ? 'opacity-100 animate-ball-bump' : 'opacity-0'}`}
          >
            <Image
